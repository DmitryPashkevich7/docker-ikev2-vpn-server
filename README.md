...
Recipe to build [`shrlkholmes/ikev2-vpn-server`](https://registry.hub.docker.com/r/shrlkholmes/docker-ikev2-vpn-server/) Docker image.

## Usage

### 1. Start the IKEv2 VPN Server

    docker run --privileged -d --name ikev2-vpn-server --restart=always -p 500:500/udp -p 4500:4500/udp shrlkholmes/docker-ikev2-vpn-server:0.3.0

### 2. Generate the .mobileconfig (for iOS / macOS)

    docker run --privileged -i -t --rm --volumes-from ikev2-vpn-server -e "HOST=vpn1.example.com" shrlkholmes/docker-ikev2-vpn-server:0.3.0 generate-mobileconfig > ikev2-vpn.mobileconfig
